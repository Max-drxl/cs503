...
int get_student(int fd, int id, student_t *s)
{
    ...
    lseekOffset = lseek(fd, myoffset, SEEK_SET);
    if (lseekOffset < 0 || lseekOffset < myoffset) {
        return ERR_DB_FILE;
    }

    //load 's' buffer with existing record
    bytes_read = read(fd, s, STUDENT_RECORD_SIZE);
    if (bytes_read < 0) {
        return ERR_DB_FILE;
    }

    //byte_compare > 0 means 's' has data that is not just zeroes
    byte_compare = memcmp(s, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
    if (!byte_compare > 0) {
...
{
    //new-up, make another 64 bytes available
    student_t mystudent = EMPTY_STUDENT_RECORD;
    off_t myoffset = 0; 
    off_t lseekOffset = 0;
    int byte_compare = 0;
    ...
    //use the normal empty student record buffer to compare to
    //the mystudent loaded buffer from 'read'
    byte_compare = memcmp(&mystudent, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
    if (byte_compare != 0) {
    ...
    //write using pointer to mystudent and the record size
    //write converts pointer to stream and prints as a bytestream
    bytes_written = write(fd, &mystudent, STUDENT_RECORD_SIZE);
...
int del_student(int fd, int id)
{
    ...
    //this loads the buffer for mystudent from db
    check_student = get_student(fd, id, &mystudent);
    if (check_student != 0) {
    ...
    //replace loaded buffer with zeroes and write zeroes
    //memcpy returns so using it here instead of declaring new variable
    bytes_written = write(fd, memcpy(&mystudent,&EMPTY_STUDENT_RECORD,STUDENT_RECORD_SIZE), STUDENT_RECORD_SIZE);
    if (bytes_written < 0 || bytes_written < STUDENT_RECORD_SIZE) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_OP;
    }
...
{
    student_t mystudent = EMPTY_STUDENT_RECORD;
    float calculated_gpa_from_student = 0.0;
    ...
    while (fd > 0) {
        ...
        if (byte_compare != 0) {
            ...
            record_count++;
            calculated_gpa_from_student = mystudent.gpa/100.0;
            printf(STUDENT_PRINT_FMT_STRING, mystudent.id, mystudent.fname, mystudent.lname, calculated_gpa_from_student);
        }
    }
    if (record_count == 0) {
        printf(M_DB_EMPTY);
...
void print_student(student_t *s)
{
    ...
    float calculated_gpa_from_s = 0.0;
