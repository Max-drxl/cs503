...
// Prototypes are provided for the functions to keep the code 
//      more logically organized such that functions are
//      visible immediately but defined elsewhere. This also
//      assists the compiler with both generating correct code
//      and catching mistakes. This is also considered best practice.
void  usage(char *);
...
int count_words(char *str){
    ...
    for(int i=0; i < len; i++) {
        if (word_start) {
            if (str[i] == SPACE_CHAR) {
                word_start = false;     //found space at end of word
            } else {
                continue;
            }
        } else {
...
//          because at index str[len(str)] is the '\0' that we want
...
void  reverse_string(char *str){
    ...
    int start_idx = 0;                  //start of string index
    char tmp_char;                      //temporary character storage

    while (end_idx > start_idx) {
        tmp_char = str[start_idx];      //required for either start or end
...
//      1. increment wc, and set word_start to true like before
...
void  word_print(char *str){
    ...
    for(int i=0; i < len; i++) {
        if (word_start) {
            if (str[i] == SPACE_CHAR) {       //end of current word
                printf(" (%d)\n", wlen);
                word_start = false;
                wlen = 0;
            } else if (i == last_char_idx) {  //last char in string processing
                wlen++;
                printf("%c", str[i]);
                printf(" (%d)\n", wlen);
                return;
            } else {
                printf("%c", str[i]);         //this prints all but first and last
                wlen++;
            }
        } else {
            if (str[i] == SPACE_CHAR) {       //only hit if double-space in input
                //printf(" test(%d)\n", wlen);     lines 121-127, unnecessary
                //word_start = false;              lines 121-128, already false
                wlen = 0;
            } else {                          //first char in word processing
                wc++;               
                word_start = true;
                printf("%d. ", wc);
                printf("%c", str[i]);
    ...
    printf(" (%d)\n", wlen);        //adding catchall for single-char input
    return;
}
...
int main(int argc, char *argv[]){
    ...
    switch (opt){
        ...
        case 'r':
            ...
            //The string is reversed in place because we have already
            //allocated the memory for the string with all the 
            //original characters so we can simply swap the characters
            //of the string in memory without needing to use any
            //additional memory outside of the variables.
            break;
        case 'w':
            printf("Word Print\n----------\n");
        ...
        //The default option is to catch any unexpected input from
        //the user and prevent the program from misinterpreting it.
        //GIGO.
        default:
            usage(argv[0]);
    ...
    //We placed the exit function on the default option because
    //we have no try-again handling in place to do anything with
    //an invalid option so the safest thing to do is to force
    //terminate the program and free the memory.
    //We use break statements elsewhere to ensure that the control
    //flow of the program continues and ultimately exits normally
    //without falling through from one case to another - which
    //would happen without the break statements.
}
